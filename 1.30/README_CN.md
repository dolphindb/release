...
* 优化以下函数调用 `rolling` 函数时的计算性能：`cumsum`, `cummax`, `cummin`, `cumprod`, `mcount` 等。（**1.30.19**）
...
* `createReactiveStateEngine` 支持状态函数 `trueRange`。（**1.30.19**）
...
* 对 DECIMAL 类型列进行 `pivot by`，且对 `select` 指定的列进行 `rowSum` 等分布式计算时，查询结果不正确。（**1.30.21.3**）
