...
* 日志中新增登入、登出相关信息包括登录用户、IP、端口、登录状态等。（**1.30.22.4**）
...
* 当 `database` 的 *atomic*='CHUNK' 时，若一个事务涉及多个 chunk，可能出现元数据版本不一致。（**1.30.22.2**）
* 将非字符串变量传入 `interval` 函数的 *label* 参数，导致服务器崩溃。（**1.30.22.2**）
* 查询以时间列进行值分区的表时，通过 `where` 条件对分区列进行过滤的耗时比较长，此为1.30.22版本引入的问题。（**1.30.22.2**）
* `mprod` 函数的中间结果溢出导致服务器宕机。（**1.30.22.2**）
* 并发执行 `restore` 事务或 `restore` 事务与其他事务并发执行时，重启后可能导致元数据不正确。（**1.30.22.2**）
...
* 1.30.22版本，使用例如 `... and not like(id, '%a')，not like, not in, not between` 的语句时，解析会出现报错。（**1.30.22.1**）
...
### GUI
  ...
  * 新增支持展示 Data Browser 中某一单元格对应的所有数据功能。（**1.30.22.2**）
  * 新增支持加密存储用户名、密码等敏感信息。（**1.30.22.2**）
  * 新增支持 Log 打印过长信息自动换行功能。（**1.30.22.2**）
  * 优化了连接失败时的报错信息。（**1.30.22.2**）
  * 优化了同步文件到 server 失败时的报错信息，将说明具体失败原因。（**1.30.22.2**）
  * 优化了使用 `plot` 函数生成图像时：（**1.30.22.2**）
    * 若横坐标为时间类型，右侧数轴保留。
    * 删去了折线图中折点默认添加的符号。
  * Log 和 Data Browser 中若展示负数或 DECIMAL32/64/128类型数据，整数部分使用标准的千位分隔符（逗号）。（**1.30.22.2**）
  * 修复 Python Parser 解析模式下创建维度表报错的问题。（**1.30.22.2**）
  * 修复若 Data Browser 中某一单元格数据为负数，其拷贝结果不正确的问题。（**1.30.22.2**）
  * 修复连接失败时 GUI 冻结的问题，新增了重连时间限制。（**1.30.22.2**）
  * 修复 `rename` 文件名后实际文件系统中未对应修改的问题。（**1.30.22.2**）
...
### C++ API
  ...
  - `MultithreadedTableWriter` 新增支持断开后自动重连。（**1.30.22.1**）
  - 新增支持创建 Void Vector 型数据。（**1.30.22.1**）
  - 新增支持 select NULL 语句。（**1.30.22.1**）
  - 新增支持数据类型 DECIMAL128。（**1.30.22.1**）
  - `DBConnection` 类的 run, upload 接口中新增锁以防止并发调用时宕机。（**1.30.22.1**）
  - `MultithreadedTableWriter` 的 `insert` 方法新增支持插入char[] 型数据 ，且可将其转换为字符串。（**1.30.22.1**）
  - CmakeLists.txt 文件中新增编译选项 *USE_OPENSSL*，可选择是否链接 OpenSSL 库。（**1.30.22.1**）
  - `Table` 类新增 `clear()` 方法，可用来清除表中数据。（**1.30.22.1**）
  - 修复高可用中偶现的重复提交写入任务的问题。（**1.30.22.1**）
  - 修复流订阅中客户端偶现解析消息失败的问题。（**1.30.22.1**）
  - 修复流订阅跨进程共享内存表时偶现的使用已失效指针的问题。（**1.30.22.1**）
  - 修复使用高版本 OpenSSL 时因使用不建议的函数而出现的编译警告。（**1.30.22.1**）
  - 优化了 `MultithreadedTableWriter` 的 `insert` 方法的报错信息。（**1.30.22.1**）
  - 取消下载包含 string/symbol 型 vector 数据时的最大长度限制。（**1.30.22.1**）
  - 新增在上传 symbol/string 型数据时先检验数据长度，若不小于 256K 则会抛出异常。（**1.30.22.1**）
  - `MultithreadedTableWriter` 新增回调接口。（**1.30.21.1**）
...
### Go API
  ...
  - 新增支持数据类型 Decimal128。（**1.30.22**）
  - 新增支持连接高可用。（**1.30.22**）
  - 新增支持订阅异构流表。（**1.30.22**）
  - SubscribeRequest 新增支持 msgAsTable 选项。（**1.30.22**）
  - 修复了流订阅丢失初始数据的问题。（**1.30.22**）
  - 新增 Vector 的 GetRawValue 方法，用于统一不同类型 Vector 的真实输出数值。（**1.30.21**）
  - 新增支持流数据订阅端主动建立 TCP 连接。（**1.30.21**）
  - 修复使用 Datatype 输出字符串时，数值类型错误显示为科学计算法的问题。（**1.30.21**）
  - 修复当一个 Table 内存在多个 SymbolExtend 时，发生取值错误的问题。（**1.30.21**）
